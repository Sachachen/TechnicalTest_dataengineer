...
def main():
    ...
    # Simple for-loop behavior:
    # - log_type == 'all'  => generate ids, access, endpoint in sequence
    # - otherwise          => generate only the selected log type
    if str(log_type).lower() == 'all':
        log_types = ['ids', 'access', 'endpoint']
    else:
        log_types = [str(log_type).lower()]

    for current_log_type in log_types:

        if current_log_type == 'ids':
        ...
        elif current_log_type == 'endpoint':
            ...
            raise ValueError(f"Unsupported log_type: {current_log_type}")
